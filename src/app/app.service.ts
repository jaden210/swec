...
<<<<<<< HEAD
    if (isAdmin) {
      // window.open("https://app.cal.com/availability");
      this._bottomSheet.open(AdminAppointmentSheetComponent, {
        disableClose: true
      });
    } else {
      this._bottomSheet.open(AppointmentSheetComponent, {
        disableClose: true,
        panelClass: this.checkMobile() ? 'full-height' : '',
        data: {isAdmin}
      });
    }
=======
...
<<<<<<< HEAD
  public getAppointmentsByName(name: string): Observable<Appointment[]> {
    return this._db.collection<Appointment>(`appointments`, ref => ref.where("name", "==", name).orderBy("appointment", "desc")).valueChanges({idField: 'id'});
  }
